...
- **인코딩 표준**:
  ...
  - 모든 소스 코드 및 PowerShell(`.ps1`): **UTF-8 (no BOM)** (Windows PowerShell 5.1/7.x 공통 호환성 보장)
...
- **세션 초기화 (Session Bootstrap)**: 새 터미널 세션 시작 시 UTF8 인코딩 설정 및 `$ProgressPreference = 'SilentlyContinue'`를 강제합니다.
  ...
  ```
- **NoProfile Mode**: 에이전트 내부에서 실행되는 모든 PowerShell 호출은 로컬 세션 프로필(`Microsoft.PowerShell_profile.ps1`)의 간섭을 배제하기 위해 `-NoProfile` 스위치 사용을 권장합니다.
- **Shell Integration 노이즈 차단**: 터미널 출력에 포함될 수 있는 Shell Integration 시퀀스(`\e]633;...`)가 파싱 에러를 유발하는 경우, 환경 변수나 터미널 설정을 통해 이를 명시적으로 억제합니다.
- **구문 검증 명령 (Syntax Check)**: 중요한 `.ps1` 스크립트를 수정하거나 생성한 후에는 반드시 실행 전 구문 오류를 확인합니다.
  ```powershell
  Get-Content "file.ps1" | Out-String | [scriptblock]::Create($null)
  ```
...
- **터미널 Recovery SOP**: 터미널 응답이 깨지거나 파싱 불가능한 상태가 되면 즉시 `clear`, `reset` 또는 전용 초기화 스크립트를 실행하여 세션을 복구합니다.
