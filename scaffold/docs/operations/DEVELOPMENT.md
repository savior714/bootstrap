...
5. after publication and remote read-back, close the admission: `./scripts/git-safety close <task-id>` — removes only the admitted task-owned linked worktree (refused without force when the worktree is dirty or when the candidate is not contained in fresh canonical history) and then only that task's admission record. `close` owns objective Git resource safety only; it never infers semantic completion.
