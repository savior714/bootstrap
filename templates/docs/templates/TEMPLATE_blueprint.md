...
## 필수 4항 (활성 `docs/plans/PLAN_*.md` — `plan_lint` HARD)

| # | 항목 | 위치 |
| :---: | :--- | :--- |
| 1 | `## Agent Execution Pack` | `Impact Scope` **직전** — Execute Pack-only Read SSOT |
| 2 | `## Agent Completion Contract` | Execution Plan **직전** |
| 3 | `> **에이전트 스코프**:` blockquote | Execution Plan **첫 Task 직전** — Verify → Conclusion → `done` → `just plan-lint` |
| 4 | 각 Task `- **Conclusion**:` | todo/running: CSF 슬롯 · done: 실측 1줄 |

아카이브(`docs/plans/archive/**`) Blueprint는 위 4항 lint **미적용**(레거시 유지).
...
9. **마지막에서 두 번째 Task = Implementation review (-098)** · **마지막 Task = Blueprint closeout (-099)** (§「Implementation Review Task」·§「마지막 Closeout Task」 참고)

## Implementation Review Task (신규 Blueprint MUST)

구현 Task만 `done`으로 닫고 diff-first 구현 점검·Implementation Review 절을 빼먹는 사고를 막기 위해, closeout **직전**에 review Task **1개**를 둔다.

| 필드 | 규칙 |
| :--- | :--- |
| Phase | `### Phase N — Implementation review` (또는 `구현 리뷰`) — closeout Phase **직전** |
| Task-ID | `[SLUG-098]` — suffix `-098` 고정 |
| **Pre-read (MUST)** | 1. `[project_skill]` `agents/skills/review/SKILL.md` — **Execute 전 Read 필수** · 2. `[spec]` `agents/skills/plan/references/handoff-from-execute-to-review.md` · 3. `[spec]` 대상 `docs/plans/PLAN_<slug>.md` · `just plan-preread … --write`가 -098 Task에 review SKILL을 **자동 주입** |
| Goal | review/SKILL workflow로 diff-first Findings를 작성하고 `## 🔍 Implementation Review` 절을 채운다. (단일 목표 — `및`/`그리고` 금지) |
| Target | `docs/plans/PLAN_<slug>.md` (Implementation Review 절만 편집; 다른 Task `Status`/`Conclusion` 직접 수정 금지) |
| Verify | `just plan-review-gate plan=docs/plans/PLAN_<slug>.md` |
| Dependency | 직전 Phase의 **마지막 구현 Task** Task-ID |
| Conclusion (done) | `[PASS] Implementation Review 작성. plan-review-gate exit 0.` |

**예외**: 문서-only·1-Task pilot 등 **애초에 구현 Task가 0개**인 Blueprint는 review Task 생략 가능 (closeout 예외와 동일).
...
| Dependency | **Implementation review Task** `[SLUG-098]` Task-ID |
...
## Agent Execution Pack (Execute 읽기 범위 SSOT)

Blueprint **같은 파일** 안에 Execute 전용 묶음 절을 둔다. 경량 모델·Execute subagent는 **Pack + Agent Completion Contract + 각 Task `Pre-read`** 만 Read하고, 설계 판단을 다시 하지 않는다.

| Pack **포함** | Pack **제외** (설계 전용 — Execute Read 금지) |
| :--- | :--- |
| `## 🔍 Impact Scope` | `## 🎯 Origin Intent` · `Architectural Goal` |
| `## 실행 순서·선행` 표 | `## 🔍 Diagnosis & Findings` |
| `## 🛠️ Step-by-Step Execution Plan` Task 블록 전체 | `## 🏗️ Architectural Deepening` · `## 📜 Conceptual Sketch` · `## 🛡️ Risk & Strategy` |

Pack 절 헤더(`## Agent Execution Pack`)는 `Impact Scope` **직전**에 둔다. Pack 본문에 설계 요약을 다시 쓰지 않는다 — 아래 실행 절을 **그대로** 참조한다.

## 권장 (참고·복붙 OK, lint 미검증)

- `- **Closeout**:` — Task 종료 시 갱신할 Blueprint 경로 힌트
- Diagnosis / Architectural Deepening / Conceptual Sketch 등 아래 「복사용 뼈대」 설계 절 (Pack **밖**)

## Lint-first 작성 체크리스트 (`plan-lint` FAIL 방지 — Phase W4 **직전** MUST)

Blueprint 저장 후 `just plan-preread … --write` → `just plan-lint` 순서를 따르기 **전**, 아래 7항을 **작성 중**부터 지킨다. `plan-lint PASS ≠ 템플릿 품질 OK` — 이 표는 **첫 lint에서 통과**하기 위한 SSOT이다.

| # | 흔한 FAIL | 작성 시 MUST |
| :---: | :--- | :--- |
| 1 | 업무 요약·설계 절 백틱·경로 | **업무 요약**·**Origin Intent**·**Edge Case Trace**·**Diagnosis**·**Risk** — `` ` ``·`src/`·`apps/`·CLI **금지** (`plan_lint` HARD) |
| 2 | Agent scope에 Conclusion 누락 | Execution Plan **첫 Task 직전** blockquote에 **`Conclusion`**·**`plan-lint`** 문자열 **둘 다** 포함 — 예: `Verify PASS → Conclusion → just plan-task-close → just plan-lint …` |
| 3 | Task-ID·필드 `[…]` placeholder | Task-ID는 `[SLUG-001]` 형식(대문자 접두 2자+, `-`, 3자리 숫자). **`[XXX-001]`·`[SLUG-001]`·`[TBD]`·한글 `[…]` 슬롯 금지** — `plan_lint`가 XXX/SLUG **HARD FAIL** |
| 4 | `Labels: plan` / `docs` / unknown | **`plan`·`type-safety`·`tech-debt` HARD FAIL**. active root에서 **`docs` HARD FAIL** → **`Improvement`**. **`refactor`** alias→Improvement(통과). 내부·도구 → **`Improvement`** 또는 **`tooling`** |
| 5 | Target이 디렉터리 | **`Target` = 파일 1개** — `path/to/file.ts` ✅ · `path/to/dir/` ❌ · Impact Scope 표와 **동일 경로** |
| 6 | Pre-read 마커 없음 | **`just plan-preread docs/plans/<file>.md --write`** — `docs/plans/PLAN_*.md` Target(0.1·9.9)은 **fallback Pre-read 자동 주입** |
| 7 | Task 9.9 Goal 줄바꿈 | **Goal 한 줄** — `` `## 🔁 …` `` 헤딩·`- **Goal**:` 다음 continuation line **금지** |

**CLI 순서 (고정)**: Target·Task-ID·Labels 확정 → `plan-preread --write` → `just plan-lint` (contract, 초안 저장) → exit 0 → 구현 착수 전 `just plan-lint-strict` → exit 0.

### plan-lint 2-tier 게이트 (contract vs strict)

| 게이트 | 명령 | 용도 | warnings |
| :--- | :--- | :--- | :--- |
| **contract** | `just plan-lint docs/plans/<file>.md` | Blueprint 초안 저장·구조 검증 | exit 0 |
| **strict** | `just plan-lint-strict docs/plans/<file>.md` | 구현 착수·closeout 전 품질·거버넌스 | exit 1 |
| **quality** (선택) | `just plan-lint-quality docs/plans/<file>.md` | #8–#10 품질 게이트 추가 | strict와 동일 |

## MUST NOT

- **Linear-Issue 누락**: `Linear-Issue` 필드가 없거나 비어 있으면 lint FAIL (단, `Linear-Policy: internal` 명시 시 예외)
- Task 제목·Goal에 `(선택)` `(Optional)` `(필요 시)`
- **Task Goal 작성 SSOT**: [task-decomposition.md](../../agents/skills/plan/references/task-decomposition.md) §Task Goal 작성 SSOT — BAD/GOOD 3종·작성 공식·체크리스트·예외 목록 (아래 MUST NOT과 함께 참고)
...
- **DoD(Definition of Done)의 `just <recipe>`는 루트 Justfile 및 `import`된 `.just/*.just` 모듈에 존재해야 함**: unknown recipe면 **`plan_lint` HARD** ([`justfile_recipes.py`](../../scripts/plan_loop/plan_lint/justfile_recipes.py)).
...
- **Priority**: 1
- **Labels**: Improvement  <!-- MUST: allowlist만 — `plan`/`docs` 금지. 제품 기능→Feature, 내부·도구→Improvement 또는 tooling(alias) -->
- **Architectural Goal**: …
...
| {예: API 404·시드 없음} | Risk | SLUG-002 | Red 테스트 선행 |
| {예: 빈 대기 목록 UI} | Origin | SLUG-003 | |

## 🧭 Context Pre-read Gate (실행 전 필수)

> ⚠️ **에이전트 주의**: `just plan-lint` **전에** 반드시 `just plan-preread docs/plans/PLAN_xxx.md --write`를 실행한다. 수동 Pre-read 초안만 두면 **전 Task** `missing Task-level Pre-read` FAIL.

<!-- plan-preread:v1 paths=0 must_read_installed=0 -->
(planned: `just plan-preread docs/plans/PLAN_xxx.md --write` — 위 HTML 주석은 CLI가 `--write` 시 갱신)
...
> **에이전트 스코프**: Task 1개씩. `Verify` PASS → **Conclusion** 기입 → `just plan-task-close plan=docs/plans/PLAN_xxx.md task=… conclusion="…"` → **`just plan-lint docs/plans/PLAN_xxx.md`**. Review Task(-098)에서 Implementation Review 작성 후 `just plan-review-gate`. Closeout Task에서 Roll-up 후 `just plan-close` Verify.

### Phase 0 — Edge case gap audit

#### Task 0.1: Edge Case Trace 갭 감사 및 보완 Task 반영 [Unit: Atomic]
- Task-ID: [SLUG-001] | Linear-Issue: TEM-XXX | Status: todo | Priority: 1 | Labels: Improvement | RetryPolicy: none
- **Pre-read**: 이 Task만 — `write`/`patch` 전 **전부** Read <!-- plan-preread --write 시 자동 주입(docs/plans Target fallback) -->
- **Action**: Edit File | **Target**: `docs/plans/PLAN_xxx.md`
- **Closeout**: `docs/plans/PLAN_xxx.md` (Task SLUG-001 `Conclusion`·`Status`)
...
- Task-ID: [SLUG-002] | Linear-Issue: TEM-XXX | Status: todo | Priority: N | Labels: Improvement | RetryPolicy: none
- **Pre-read**: 이 Task만 — `write`/`patch` 전 **전부** Read <!-- plan-preread --write 시 자동 주입 -->
- **Action**: Edit File | **Target**: `path/to/code.py` <!-- MUST: 파일 1개 — `path/to/dir/` 금지 -->
- **Closeout**: `docs/plans/PLAN_xxx.md` (Task SLUG-002 `Conclusion`·`Status`)
- **Goal**: … | **Diagnostics**: 0
...
- Task-ID: [SLUG-098] | Linear-Issue: TEM-XXX | Status: todo | Priority: 3 | Labels: Improvement | RetryPolicy: none
- **Pre-read**: 이 Task만 — `write`/`patch` 전 **전부** Read <!-- plan-preread --write 시 review SKILL 자동 주입 -->
  1. `[project_skill]` `agents/skills/review/SKILL.md`
  2. `[spec]` `agents/skills/plan/references/handoff-from-execute-to-review.md`
  3. `[spec]` `docs/plans/PLAN_xxx.md`
  4. `[rule]` `agents/core/error_patterns/detail/editing.md`
- **Action**: Edit File | **Target**: `docs/plans/PLAN_xxx.md`
- **Closeout**: `docs/plans/PLAN_xxx.md` (Task SLUG-098 `Conclusion`·`Status`)
...
- **Diagnostics**: 0
- **Verify**: `just plan-review-gate plan=docs/plans/PLAN_xxx.md`
- **Conclusion**: [판정 — 비개발자용 요약. 검증 결과]
- **Dependency**: SLUG-00N <!-- 직전 구현 Task-ID -->

### Phase 9 — Blueprint closeout

#### Task 9.9: Roll-up 작성 및 plan-close [Unit: Atomic]
- Task-ID: [SLUG-099] | Linear-Issue: TEM-XXX | Status: todo | Priority: 3 | Labels: Improvement | RetryPolicy: none
- **Pre-read**: 이 Task만 — `write`/`patch` 전 **전부** Read <!-- plan-preread --write 시 자동 주입 -->
- **Action**: Edit File | **Target**: `docs/plans/PLAN_xxx.md`
- **Closeout**: `docs/plans/PLAN_xxx.md` (Task SLUG-099 `Conclusion`·`Status`)
- **Goal**: 선행 Task Conclusion을 근거로 Conclusion and Summary Roll-up 1문단을 실측으로 작성한다. <!-- MUST: Goal 한 줄 — `## 🔁` 헤딩·줄바꿈 금지 -->
- **Diagnostics**: 0
...
- **High**: … <!-- Review Task(SLUG-098)에서 실측 작성. todo 상태 placeholder 금지 -->
- **Medium**: …
- **Low**: …

## 🔁 Conclusion & Summary

- **Roll-up**: … <!-- Closeout Task(SLUG-099)에서 실측 작성. todo 상태 placeholder 금지 -->
