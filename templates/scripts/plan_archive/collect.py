...
def plan_has_only_linear_placeholders(plan_path: Path) -> bool:
    """True when blueprint cites no real TEM-NN ids (minor fix / template-only)."""
    ...
    if not ids:
        return True
    return all(is_linear_placeholder(i) for i in ids)
...
PLAN_REF_PATTERN_WITH_BRACKETS = re.compile(r"\[?PLAN_[A-Za-z0-9_.-]+\.md\]?")
    ...
    if discussions_dir.is_dir():
        for path in sorted(discussions_dir.glob("DISCUSS_*.md")):
            ...
            if not value or value in ("", '""', "null", "~", "none"):
                continue
            # linked_plan 값에서 basename만 추출 (경로 전체일 수 있음)
            # 예: "docs/plans/PLAN_xxx.md (TEM-349)" → "PLAN_xxx.md"
            import re as _re
