...
5. **비개발자·한국어 톤** — [reporting.md](reporting.md) §1.6.0. 질문 문장에 파일 경로·식별자 나열 금지.
...
## 1.5 Stepwise Execution & Concise Reasoning

- **단계별 결과**: 각 단계마다 **간결한 결과**를 먼저 정리하고, 그 결과를 근거로 다음 단계를 진행한다.
- **짧은 reasoning**: 도구 호출 전 reasoning은 **500자 이내**로 유지한다.
- **근거 보강**: 다음 단계 방향이 reasoning에서 나오지 않으면 가정·실측·성공 기준을 **보강**한 뒤 진행한다.

## 1.6 Pythonic Integrity (Ruff Standards)
상세: [backend/python.md](../domains/backend/python.md)

## 1.7 Workaround Accountability & Close Turn Reflection
...
## 1.8 Code Quality Lifecycle (시점별 품질)
...
- **커밋 게이트**: `commit-gate`(Husky pre-commit) 실패 시 **반드시 오류를 수정**하고 재시도한다. `git commit --no-verify`로 우회하는 것은 **절대 금지**된다. (`just lint-fix` 후 `just commit-gate` 수동 재시도)
