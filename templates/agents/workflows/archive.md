---
situation: 플랜 아카이브
# trigger: /archive  ← catalog metadata only; Read this file before executing (error_patterns/detail/workflow.md#161)
level: Recommended
...
6. **[자동화] Unified Sync (플랜 이동 전 1회)**: `archive` 명령이 **첫 플랜 파일 이동 전**에 **`just sync --check`와 동일한** `scripts/agent/sync.py --check`를 자동 실행한다 (code-lock 해시 + 스펙 역검증). FAIL이면 파일이 archive로 옮겨지지 않으며, Phase 2에서 누락된 `docs/specs/` 갱신 후 재실행한다. 문서화된 사유가 있을 때만 `--skip-unified-sync`(이동 전 검사도 생략)를 쓰고, **사후** `just sync --check`로 재검사한다. 수동 점검: `just sync --check` → drift·후보 스펙 확인 → 필요 시 Phase 2 본문 갱신 → `archive` 재실행
7. **Git 반영**
...
- **Unified Sync**: [sync.md](sync.md) · `just sync`
