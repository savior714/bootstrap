...
```
docs/plans/PLAN_playwright_<scope>_<slug>.md
```

**필수 준수 사항**:
1. [TEMPLATE_blueprint.md](../../docs/templates/TEMPLATE_blueprint.md) · [plan/SKILL.md](../skills/plan/SKILL.md) 작성 모드
2. `Origin Intent` 출처: `/playwright` — scope·탐색 Findings
3. `just plan-preread` → `just plan-lint` PASS (`uv run python scripts/plan_loop/plan_lint.py` 동등)
4. **코드 수정 금지** — Blueprint만

### 5단계 — 핸드오프 (Same-session)

**SSOT**: [plan/handoff-contract.md](../skills/plan/references/handoff-contract.md) §메뉴 B.

- 산출물 요약 턴(18줄 이내) + **이 PLAN 전체 순차 실행** `(권장)` AskQuestion
- **`@` + `/plan` 안내 금지**
- (메뉴 B Execute 선택 시) Closeout 후 **review** MUST — [handoff-from-execute-to-review.md](../skills/plan/references/handoff-from-execute-to-review.md)
