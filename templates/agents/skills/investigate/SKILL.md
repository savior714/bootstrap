...
> **실행 스코프 (MUST)**: investigate는 **메인 에이전트가 직접 수행** — subagent(Task) 위임 **하지 않음**. 로그·스택·diff·테스트·just·DB·hub 로그를 직접 실행·조회해 가설과 범위를 좁히므로, 메인 직접 실행이 적절하다. O2 이관 트리거(파일 1개·함수 1개 포함)가 해당되지만, investigate는 **직접 수행 carve-out** — 메인 `Read`/`Grep`/`Shell` 직접 사용 허용.
