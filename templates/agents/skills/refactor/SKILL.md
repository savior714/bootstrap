...
description: >
  ...
  문제와 설계 방향을 명확히 하고, 같은 세션에서 Blueprint(plan-lint PASS)까지 이어간다.
license: MIT
metadata:
  version: "1.1.0"
...
실제 소스 코드를 수정하지 않으며, 설계가 끝나면 **같은 세션**에서 [plan](../plan/SKILL.md) 작성 모드로 `docs/plans/PLAN_*.md`·`plan-lint` PASS까지 이어간 뒤 RGR 패치는 plan **Execute**에서 수행한다.
...
검증 설계(3단계)까지 합의가 끝나면 **Close `AskQuestion` 1턴** 후 다음으로 이어간다.

**핸드오프 SSOT**: [references/handoff-to-plan.md](references/handoff-to-plan.md) — same-session plan · 3단계→Blueprint 매핑 · 산출물 요약 턴 · 공통 [plan/handoff-contract.md](../plan/references/handoff-contract.md).

| 선택 | 에이전트 행동 |
| :--- | :--- |
| **실행 계획(Blueprint)으로 만들기** `(권장)` | **같은 세션**에서 [plan/SKILL.md](../plan/SKILL.md) 작성 모드 실행 — `PLAN_*.md` · `plan-preread` · `plan-lint` PASS까지. **사용자에게 `/plan` 입력을 시키지 않음**. |
| **이 주제 더 논의하기** | 3단계 중 해당 단계로 복귀 — PLAN 미작성 |
| **여기서 마무리** | 노트·채팅만 정리 |

**금지**: RGR 미합의 시 Blueprint 작성 · plan-lint PASS 전 「설계 완료」·「`/plan` 하세요」만 텍스트로 안내 · **같은 주제** PLAN 재작성.

**이미 `PLAN_*.md` + plan-lint PASS**인 주제 재진입 → refactor **금지** — [plan/SKILL.md Execute](../plan/SKILL.md)로 위임.
