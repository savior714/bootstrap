...
description: >
  ...
  매 세션 Context7 MCP로 증상·스택에 맞는 최신 framework 디버그 문서를 조회해 가설·근본 원인 보조(always-on).
...
> **실행 스코프 (MUST)**: diagnose는 **메인 에이전트가 직접 수행** — subagent(Task) 위임 **하지 않음**. Phase 1 피드백 루프(vitest·just·hub 로그)는 반복 실행이 필요하고, 수정+회귀 테스트는 원자 편집이므로 메인 직접 실행이 적절하다. O2 이관 트리거(파일 1개·함수 1개 포함)가 해당되지만, diagnose는 **직접 수행 carve-out** — 메인 `StrReplace`/`Write`/Shell 직접 사용 허용.
...
2. **실행 후 보고** — 루프·테스트·just·hub 로그·**Context7**(always-on)은 에이전트가 **직접** 돌린 뒤 보고한다.
3. **추측 패치 금지** — 계측·반증 전에 소스 수정하지 않는다(Phase 5만 수정).
4. **회귀 테스트** — correct seam이 있으면 fix **전에** failing test를 만든다.
5. **Context7 always-on** — Phase 2 직후 [context7-library-check.md](references/context7-library-check.md): `resolve-library-id` + `query-docs` ≥1. MCP 불가 시 SKIP 기록 — **미재현·도메인-only도 생략 금지**. Context7만으로 근본 원인 확정 **금지**.
...
| **Context7 (always-on)** | MCP `user-context7` — [context7-library-check.md](references/context7-library-check.md). Phase 2 후·Phase 3 전. **실행한 검증** 표 필수 |
...
## Phase 2.5 — Context7 (always-on, MUST)

[context7-library-check.md](references/context7-library-check.md) — Phase 3 **직전**:

1. 실패 파일·스택·증상 → 주 `libraryName` (§2)
2. `resolve-library-id` → `query-docs` ≥1 (세션 max 2)
3. **실행한 검증** 표에 기록 · Phase 3 가설·Root Cause에 **루프·probe와 연결**해 인용

MCP SKIP 가능 — Phase 3 진행 **중단하지 않음**. Context7만으로 fix **금지**.
...
close 턴은 **반드시** AskQuestion/`question` tool call — 텍스트만 "골라주세요" 금지.

### AskQuestion unavailable — pending_ask (MUST)

도구 미노출·실패 시 [pending-ask-fallback.md](references/pending-ask-fallback.md) — Fix 완료 / 미재현 / investigate 모드별 YAML frontmatter + `askquestion_attempts` 로그.
...
표 필수 — vitest/just/hub/DB/bash loop/**Context7** 등
...
**루프 만들고, 직접 돌리고, Context7 조회하고, 계측하고, 고치고, 회귀 테스트하고, 한국어로 마무리한다.** 조사만은 investigate.
