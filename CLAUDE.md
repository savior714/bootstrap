...
- **[CRITICAL] Tool-First & Zero-Shell Discovery & Navigation**: 파일 탐색, 검색, 목록 조회 및 **경로 이동** 시 OS 쉘 명령어(`dir`, `ls`, `find`, `Get-ChildItem`, `grep`, `cd`, `pushd`)의 **사용을 전면 금지**합니다. 에이전트는 반드시 **IDE 전용 구조화 도구(Glob, Grep, Read 등)**만을 사용해야 합니다.
  ...
  - **예외**: 도구가 물리적으로 지원하지 않는 '특수 바이너리 실행'이나 '프로세스 제어' 시에만 쉘을 허용하며, 이 경우에도 `ls`/`dir` 등 **탐색형 명령어는 절대 혼용 금지**합니다. 실행 위치 제어는 반드시 도구의 **`Cwd` 매개변수**를 사용합니다.
...
- [ ] **Type Check Zero**: `.ts`/`.tsx` 파일 수정 시 `npm run type-check` 에러 0개를 확인했는가? 에러 존재 시 `scripts/type-check-slice.ps1`로 Error-Only Context를 추출하여 LLM에 전달했는가?
- [ ] **No @Codebase**: 바이브 코딩 세션에서 `@Codebase` 대신 `@Symbols`만 사용했는가? `src/domain/index.ts` Barrel Export를 단일 타입 컨텍스트 진입점으로 사용했는가?
...
|    6     | `docs/TS_TYPE_VALIDATION.md`      | **TypeScript 타입 검증 전략**    | Error-Only Context, Schema-First, 3-IDE 적용    |
|    7     | `docs/TS_ADVANCED_PATTERNS.md`    | **고급 타입 패턴**               | DDD 타입 분리, Symbol Reference, Type Flatten   |
|    8     | `docs/VIBE_CODING_PROTOCOL.md`    | **바이브 코딩 프로토콜**         | Validate-and-Prune, L1/L2/L3, Agent-to-Agent   |
...
- **TypeScript 타입 에러 발생 시** → `npm run type-check` 실행 → Error-Only Context 추출(`scripts/type-check-slice.ps1`) → LLM에 최소 컨텍스트만 전달. 전략 SSOT: `docs/TS_TYPE_VALIDATION.md`.
- **바이브 코딩 세션 시** → `@Codebase` 사용 절대 금지. `@Symbols` + L1/L2/L3 슬라이싱 적용. 프로토콜 SSOT: `docs/VIBE_CODING_PROTOCOL.md`.
...
- TypeScript 타입 검증 전략: [`docs/TS_TYPE_VALIDATION.md`](docs/TS_TYPE_VALIDATION.md)
- 바이브 코딩 프로토콜: [`docs/VIBE_CODING_PROTOCOL.md`](docs/VIBE_CODING_PROTOCOL.md)
